...
class types(Enum):
    LOAD,STORE,ADD,SUB,MUL,DIV,SHL,SHR,AND,OR,XOR,NOT,PUSH,POP,JLE,JZ,JMP,CALL,SUCCESS,FAIL,RET=[i for i in range(14)]

class Ts(Enum):
    ARITH,STACK_ARITH,J,END=[i for i in range(2)]
...
class Translator():
    ...
    def process_LOAD(self,code:bytes,numtype:str):
        ...
        if numtype=="RR":
            ...
            suffix='b'
        encoding,_=ks.asm(f'mov rax, {imm};mov {self.r[code[1]]}{suffix}, [rax]')
        return bytes(encoding),len(encoding)
    ...
    def process_STORE(self,code:bytes,numtype:str):
        ...
        if numtype=="RR":
            ...
            suffix='b'
        encoding,_=ks.asm(f'mov rax, {imm};mov [rax], {self.r[code[1]]}{suffix}')
        return bytes(encoding),len(encoding)
    ...
    def process_ARITH(self,code:bytes,type:types,numtype:str):
        ...
        if numtype=="R":
            ...
            if type==types.SHR or type==types.SHL:
                encoding,_=ks.asm(f'mov cl, {self.r[code[2]]};{arith} {self.r[code[1]]}, cl')
                return bytes(encoding),len(encoding)
            else:
                encoding,_=ks.asm(f'{arith} {self.r[code[1]]}, {self.r[code[2]]}')
                return bytes(encoding),len(encoding)
        elif numtype=="RI64":
            suffix=''
        elif numtype=="RI32":
    ...
    def fill_JumpTable(self):
        for i in self.JumpTable:
            try:
                ...
                encoding,_=ks.asm(f'{instr} {self.opcodes[self.JumpTable[i]][1]-self.opcodes[i][1]}')
            except:
                print("jump to invalid address!")
                print(f"dst address in vm: {self.JumpTable[i]}")
                encoding=b''
...
class GeneralVM():
    ...
    properties={
        ...
        "pushimm64" : InstructionType(types.PUSH, "RI64"),
        "pushimm32" : InstructionType(types.PUSH, "RI32"),
        "pushimm16" : InstructionType(types.PUSH, "RI16"),
        "pushimm8" : InstructionType(types.PUSH, "RI8"),
        "pushreg" : InstructionType(types.PUSH, "RR"),

        "popreg" : InstructionType(types.POP, "RR"),
    ...
    def preprocess(self, codes:bytes):
        ...
        while current_vmaddr<len(codes):
            ...
            try:
                name=self.rule[opcode]["name"]
                instr_type=self.properties[name]
                new_instruction=Instruction(instr_type,codes[current_vmaddr:current_vmaddr+instr_type.len],current_vmaddr,self.translator) 
    ...
    def to_x64(self, codes:bytes) -> bytes:
        ...
        self.opcodes={} # format: { vmaddr: [vm_instr, x86_addr, x86_instr] }
